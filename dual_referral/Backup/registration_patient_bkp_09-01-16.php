		...
		*/
		...
		private function REG_PAT_STUB(){
			...
			if(!empty($login_encrypted_key) and !empty($login_id)){
					...
					{
					   $error = array('status' => "Failed", "msg" => "Compulsory fields are empty");
					...
					else
					{
					  ...
					  if($pat_thread_id_new == "" && $doc_ref_id==0)
					  {
						  ...
						  $sql_print="insert into patient_stub (Patient_Name, Patient_Age, Patient_Gender, Patient_Location, Patient_mobile_number, Patient_issue_notes, Reg_by_doc, Patient_defined_notes, doc_ref_id) values('$Patient_Name','$Patient_Age','$Patient_Gender','$Patient_Location','$Patient_mobile_number','$Patient_issue','$login_id','$Patient_note','$doc_ref_id')";
					  
						  //$this->logger->write("INFO :","Saving the patient sql_print sql_print".$sql_print); 
					  
						  $sql_insert_save=mysql_query("insert into patient_stub (Patient_Name, Patient_Age, Patient_Gender, Patient_Location, Patient_mobile_number, Patient_issue_notes, Reg_by_doc, Patient_defined_notes, doc_ref_id) values('$Patient_Name','$Patient_Age','$Patient_Gender','$Patient_Location','$Patient_mobile_number','$Patient_issue','$login_id','$Patient_note','$doc_ref_id')",$this->db);
						  
						  if($sql_insert_save=='success'){
						     $success = array('status' => "Success", "msg" => "Patient successfully registered");
						     $this->response($this->json($success), 200);
						  }
						  else
						  {
						    $error = array('status' => "Failure", "msg" => "Patient could not be saved.SQL exception expected");
					  ...
					  if($pat_thread_id_new == "" && $doc_ref_id !=0)
					  {
						  ...
						  if(mysql_num_rows($res_get_hospital)==1)
						  {
						      ...
						      if(mysql_num_rows($res_referral_map)==1){
						      	...
						      	$error = array('status' => "Failure", "msg" => "Some issue with hospital user mapping");
						      	$this->response($this->json($error), 400);
						      }
					  ...
					  if($sql_insert=='success')
								...
								$sql_gcm=mysql_query("select mobile_os_type,gcm_regid from gcm_users gs where gs.mob_number=(select Doctor_mobile_number from doctor_stub where Doctor_serial_id='$doc_ref_id')",$this->db);
								 ...
								 if($num > 0){
									...
									if($mobile_os == 'Android'){
									    ...
									    $registatoin_ids = array($gcm_id);
									    $message = array("msg" => "DR.".$doctor_name." has reffered a patient ".$Patient_Name,"flag_push"=> "Refer", "Refer_In_cnt" => $cnt_refer_in);
									    $result = $this->gcm->send_notification($registatoin_ids, $message);
									    ...
									    $extra =array("msg" => "DR.".$doctor_name." has reffered a patient ".$Patient_Name,"flag_push"=> "Refer", "Refer_In_cnt" => $cnt_refer_in);
						 ...
						 {
						   $error = array('status' => "Failure", "msg" => "Patient could not be referred");
						   $this->response($this->json($success), 400);
						 }
				 ...
				 {
				    $error = array('status' => "Failed", "msg" => "Authentication failed with the encrypted key");
					$this->response($this->json($error), 400);
				 }
			...
			// If invalid inputs "Bad Request" status message and reason
				$error = array('status' => "Failed", "msg" => "Encrypted key or id is empty");
				$this->response($this->json($error), 400);
			}
