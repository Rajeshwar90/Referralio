...
if (mysql_num_rows ( $result_chk ) > 0) {
    $success = array (
            'status' => "Success",
            "msg" => "Referring Doctor already mapped to one of our hospital user" 
    );
    echo json_encode ( $success );
} else {
    
    $insert_status = "INSERT INTO referral_mapping(hospital_id,hospital_user_id,referring_doctor_id) VALUES ('$hospital_id','$hospital_user_id','$referring_doc_id')";
    $result = mysql_query ( $insert_status );
    
    if ($result) {
        
        $success = array (
                'status' => "Success",
                "msg" => "Referring Doctor mapped successfully" 
        );
        echo json_encode ( $success );
    } else {
        $success = array (
                'status' => "Failure",
                "msg" => "Referring Doctor could not be mapped" 
        );
        echo json_encode ( $success );
    }
}
