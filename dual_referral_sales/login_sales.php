<?php
	...
	class LOGIN_SALES extends REST {
		...
		*/
		...
		private function LOGIN_SALES(){
		    error_reporting(0);
		    //$this->logger->write("INFO :","Calling LOGIN for sales".$this->server.",username".$this->username.",connection=>".$this->db);
			...
			$mob_number = $post['sales_mobile_number'];
			$password= $post['pwd'];
			
				
			$this->logger->write("INFO :","login with".$mob_number."pass".$password."pass".$this->db);
			...
			if(!empty($mob_number) and !empty($password)){
					$sql_query = "SELECT * FROM doctor_stub WHERE Doctor_mobile_number = '$mob_number' AND BINARY Doctor_password = md5('$password') and type_value like '%hospital_user%' and Doctor_unregistered='False' LIMIT 1";
					
                    $sql=mysql_query($sql_query,$this->db);					
					
					$this->logger->write("INFO :","login with sql".$sql_query."=>row cnt=>".mysql_num_rows($sql));
