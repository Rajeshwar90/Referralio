		...
		private function GET_BROADCAST_MSGS(){
			...
			if(!empty($login_encrypted_key) and !empty($login_id)){
					...
					if(mysql_num_rows($sql) > 0){
						 ...
						 $query_list_doc=mysql_query("Select * from doc_broadcast_msg dm where dm.doctor_id='$login_id' UNION select * from doc_broadcast_msg dm where dm.doctor_id=0 ",$this->db);
						 
						 if(mysql_num_rows($query_list_doc)>0){
						   while($row = mysql_fetch_assoc($query_list_doc)){
									$json[] = $row;
								}
							$success = array('status' => "Success", "msg" => "Messages available","Msg_list" => $json);
					        $this->response($this->json($success), 200);
							
						 }
						 else
						 {
						    $success = array('status' => "Success", "msg" => "No Messages available");
